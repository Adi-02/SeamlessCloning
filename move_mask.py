...
class MaskMover():
    def __init__(self, image_path, mask_path):
        ...
        self.original_mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        ...
        if event == cv2.EVENT_LBUTTONDOWN:  
            ...
            if self.to_move:
                ...
                self.mask = cv2.warpAffine(self.mask,M,
                                      (self.mask.shape[1],
                                       self.mask.shape[0]), flags=cv2.INTER_NEAREST)
...
if __name__ == '__main__':
    ...
    mm = MaskMover(target_img_path, mask_path)
    offset_x, offset_y, _ = mm.move_mask()
    # Save the coordinates
    with open('offset_polygon.json', 'w') as file:
        json.dump((offset_x,offset_y), file)
